...
export const generateHallTicket = async (students, options = {}) => {
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    pdf.text(student.school || "", MARGIN + 27, y);
    ...
    const badgeX = MARGIN + CONTENT_W - (boxSize + boxPadding + congratsWidth);
    ...
    const venueExtraHeight = Math.max(0, (venueLines.length - 1) * 5);
    
    pdf.roundedRect(MARGIN + 7, y, CONTENT_W - 14, 46 + venueExtraHeight, 1.5, 1.5, 'FD');
    ...
    y += lines2.length * 6.5 + 3; // Added gap below this block
    ...
    pdf.roundedRect(MARGIN + 7, y, CONTENT_W - 14, 13, 1.5, 1.5, 'FD');
    pdf.setLineDashPattern([], 0); // Reset dash
    ...
    pdf.text(bannerLines, WIDTH / 2, y + 5.5, { align: 'center' });
  }
